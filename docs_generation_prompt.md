...
Mon objectif est de créer un site statique **MkDocs** (thème Material) entièrement en **Français** pour permettre aux utilisateurs de visualiser l'architecture et de télécharger les fichiers de fabrication.
...
4.  **Documentation Firmware (`docs/firmware.md`)**
    *   Si un dossier de code source est présent (ex: `Prog/source`), analyse :
        ...
        *   `hard.h` / `hard.c`: Initialisation matérielle.
        *   **`gestionentrees.c`**: Mapping des entrées (Tables `switch/case` comme `uc_EntreeTOREntreeCarte`).
        *   `slavenode.c`: Protocole de communication et vérification des données envoyées (Est-ce que les entrées sont remontées au maître ?).
    *   Crée `firmware.md` décrivant :
        *   L'environnement de build (Compilateur, MCU).
        *   La structure du projet.
        *   Le mapping mémoire (EEPROM).
        *   **Important**: Analyse et documente le **"Logical to Physical Mapping"** des entrées. Crée un tableau montrant la correspondance entre l'index logique (ex: `VAR0`) et la broche physique (ex: `RG1`). Précise si certaines entrées sont ignorées.
    *   **Diagrammes Mermaid obligatoires** :
        *   Un **Flowchart** (`graph TD`) expliquant la boucle principale et le traitement des interruptions.
        *   Un diagramme montrant le flux de traitement d'une entrée : `Bouton -> Anti-Rebond -> Stockage -> Interprétation -> Action`.
        *   *Attention à la syntaxe* : Mets des guillemets autour des labels contenant des parenthèses ou crochets (ex: `Node["Condition(1)"]`).
...
7.  **Règles Importantes**
    ...
    *   Écris tout le contenu strictement en **Français**.
    *   Vérifie bien que le support Mermaid est activé dans `mkdocs.yml`.
